...
{
    /// <summary>
    /// Предназначен для хранения одинаковых по размерам карт, исключая совпадающие по содержимому и первой букве свойства <see cref="Processor.Tag"/> одновременно.
    /// </summary>
    /// <remarks>
    ...
    public sealed class ProcessorHandler
    {
        ...
        /// <param name="p">Проверяемая карта.</param>
        /// <returns>
        /// В случае отсутствия карты в коллекции, с указанным значением первого символа (только для заглавных букв) в свойстве <see cref="Processor.Tag"/>, возвращает эту же карту, которая была подана на вход.
        /// В противном случае, метод генерирует новую карту с таким же значением свойства <see cref="Processor.Tag"/>, к которому добавлен её порядковый номер.
        /// </returns>
        /// <remarks>
        /// Рекомендуется, чтобы значения свойств <see cref="Processor.Tag"/> подаваемых на вход карт, состояли из одной заглавной буквы, т.к. остальные символы будут отброшены в любом случае.
        ...
        /// <remarks>
        /// Строковое представление текущего экземпляра получается путём преобразования первых букв значений свойств <see cref="Processor.Tag"/> хранимых карт в верхний регистр.
        /// </remarks>
        /// <seealso cref="Processor.Tag"/>
        public override string ToString() => new string(Processors.Select(p => p.Tag[0]).ToArray());
    }
}